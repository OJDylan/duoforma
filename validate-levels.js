...
function buildValidator(n) {
  ...
  function solutionValid(s) {
    ...
    for (const line of LINES) {
      ...
      for (let k = 0; k <= n - 3; k++)
        if (g[line[k]] === g[line[k + 1]] && g[line[k]] === g[line[k + 2]]) return false;
    }
    return true;
  }
  ...
  return { CELLS, solutionValid, countSolutions };
}

const validators = {
  easy: buildValidator(6),
  medium: buildValidator(6),
  hard: buildValidator(6),
  expert: buildValidator(8),
};
...
for (const diff of ["easy", "medium", "hard", "expert"]) {
  const { CELLS, solutionValid, countSolutions } = validators[diff];
  ...
  for (const lvl of data[diff]) {
    if (ids.has(lvl.id)) {
      console.log(`${diff} #${lvl.id}: duplicate id`);
      errors++;
    }
    ids.add(lvl.id);
    const sig = lvl.given + "|" + lvl.constraints.map((c) => c.join(",")).join(";");
    if (globalSigs.has(sig)) {
      console.log(`${diff} #${lvl.id}: duplicate puzzle (same as ${globalSigs.get(sig)})`);
      errors++;
  ...
  for (const lvl of data[diff]) {
    total++;
    // solution valid
    if (!solutionValid(lvl.solution)) {
      console.log(`${diff} #${lvl.id}: invalid solution`);
      errors++;
      continue;
    }
    ...
    if (bad) {
      console.log(`${diff} #${lvl.id}: given mismatch`);
      errors++;
      continue;
    }
    ...
    for (const [a, b, t] of lvl.constraints) {
      ...
      if ((t === 0) !== same) {
        console.log(`${diff} #${lvl.id}: constraint mismatch`);
        bad = true;
        break;
      }
    }
    ...
    if (n !== 1) {
      console.log(`${diff} #${lvl.id}: ${n} solutions (not unique)`);
      errors++;
    }
  }
}
